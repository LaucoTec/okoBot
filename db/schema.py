...
def iniciarBD(conn: Connection):
    ...
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_fichas_nombre_normalizado ON fichas(nombre_normalizado);''')
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_reservas_nombre_normalizado ON reservas(nombre_normalizado);''')
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_obras_nombre_normalizado ON obras(nombre_normalizado);''')
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_alias_obras_alias ON alias_obras(alias);''')
    
    # Índices para búsquedas por obra
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_fichas_id_obra ON fichas(id_obra);''')
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_reservas_id_obra ON reservas(id_obra);''')

    # Índices para optimizar consultas por propietario
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_fichas_propietario ON fichas(id_propietario);''')
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_reservas_propietario ON reservas(id_propietario);''')
    
    # Índices para búsquedas por estado y propietario
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_fichas_propietario_estado ON fichas(id_propietario, estado);''')
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_reservas_propietario_estado ON reservas(id_propietario, estado);''')
    
    # Índices para búsquedas por expiración
    conn.execute('''CREATE INDEX IF NOT EXISTS idx_reservas_expiracion ON reservas(fecha_expiracion);''')
