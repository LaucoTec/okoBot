...
class RepoObras(AsistenteDeConsultas):
    ...
    def obtenerObraPorNombre(self, nombre_obra: str):
        try:
            ...
            logger.error(f"Error obteniendo obra por nombre '{nombre_obra}': {e}", exc_info=True)
            return None
    ...
    def buscarObrasPorNombre(self, nombre_obra: str):
        try:
            ...
            logger.error(f"Error buscando obras por nombre '{nombre_obra}': {e}", exc_info=True)
            return []
    ...
    def obtenerObraPorHilo(self, id_hilo: int):
        try:
            ...
            logger.error(f"Error obteniendo obra por hilo {id_hilo}: {e}", exc_info=True)
            return None
