...
class RepoFichas(AsistenteDeConsultas):
    ...
    def crearFicha(self, id_propietario: int, nombre_personaje: str, id_obra: int, id_hilo: int, id_mensaje: int):
        try:
            ...
            cursor = self.ejecutar('''
                ...
                VALUES (?, ?, ?, ?, ?, ?);
            ''', (id_propietario, nombre_personaje, nombre_normalizado, id_obra, id_hilo, id_mensaje))

            return cursor.lastrowid if cursor is not None else None

        except sql.Error as e:
            logger.error(f"Error creando ficha para usuario {id_propietario}: {e}", exc_info=True)
            raise
    ...
    def obtenerFichaPorNombreNormalizado(self, nombre_normalizado: str):
        try:
            ...
            logger.error(f"Error obteniendo ficha por nombre '{nombre_normalizado}': {e}", exc_info=True)
            return None
    ...
    def obtenerFichasPorNombreNormalizado(self, nombre_normalizado: str):
        try:
            ...
            logger.error(f"Error obteniendo fichas por nombre '{nombre_normalizado}': {e}", exc_info=True)
            return []
    ...
    def obtenerFichasPorUsuario(self, id_propietario: int):
        try:
            ...
            logger.error(f"Error obteniendo fichas para usuario {id_propietario}: {e}", exc_info=True)
            return []
    ...
    def obtenerFichasActivasPorUsuario(self, id_propietario: int):
        try:
            ...
            logger.error(f"Error obteniendo fichas activas para usuario {id_propietario}: {e}", exc_info=True)
            return []
    ...
    def obtenerFichasPorObra(self, id_obra: int):
        try:
            ...
            logger.error(f"Error obteniendo fichas para obra {id_obra}: {e}", exc_info=True)
            return []
    ...
    def obtenerFichasPorUsuarioEstado(self, id_propietario: int, estado: str):
        try:
            ...
            logger.error(f"Error obteniendo fichas para usuario {id_propietario} con estado '{estado}': {e}", exc_info=True)
            return []
    ...
    def actualizarNombreFicha(self, id_ficha: int, nuevo_nombre: str):
        try:
            ...
            logger.error(f"Error actualizando nombre de ficha {id_ficha}: {e}", exc_info=True)
            raise
    ...
    def actualizarObraFicha(self, id_ficha: int, nuevo_id_obra: int):
        try:
            ...
            logger.error(f"Error actualizando obra de ficha {id_ficha}: {e}", exc_info=True)
            raise
    ...
    def actualizarEstadoFicha(self, id_ficha: int, nuevo_estado: str):
        ...
        try:
            ...
            logger.error(f"Error actualizando estado de ficha {id_ficha}: {e}", exc_info=True)
            raise
    ...
    def obtenerFichasEliminadasAntiguas(self, dias: int):
        try:
            ...
            logger.error(f"Error obteniendo fichas eliminadas antiguas: {e}", exc_info=True)
            return []
    ...
    def marcarFichasEliminadasPorUsuario(self, id_usuario: int):
        try:
            ...
            logger.error(f"Error marcando fichas como eliminadas para usuario {id_usuario}: {e}", exc_info=True)
            raise
    ...
    def eliminarFichaDefinitiva(self, id_ficha: int):
        try:
            ...
            logger.error(f"Error eliminando definitivamente ficha {id_ficha}: {e}", exc_info=True)
            raise