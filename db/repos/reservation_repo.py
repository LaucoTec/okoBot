...
class RepoReservas(AsistenteDeConsultas):
    ...
    def crearReserva(self, id_propietario: int, nombre_personaje: str, id_obra: int, fecha_expiracion: str, enlace_imagen: str, id_hilo: int, id_mensaje= None):
        try:
            ...
            cursor = self.ejecutar('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?);
            ''', (id_propietario, nombre_personaje, nombre_normalizado, id_obra, fecha_expiracion, enlace_imagen, id_hilo, id_mensaje))

            return cursor.lastrowid if cursor is not None else None

        except sql.Error as e:
            logger.error(f"Error creando reserva para usuario {id_propietario}: {e}", exc_info=True)
            raise
    ...
    def obtenerReservaPorNombreNormalizado(self, nombre: str):
        try:
            ...
            logger.error(f"Error obteniendo reserva por nombre '{nombre_normalizado}': {e}", exc_info=True)
            return None
    ...
    def obtenerReservasPorNombreNormalizado(self, nombre: str):
        try:
            ...
            logger.error(f"Error obteniendo reservas por nombre '{nombre_normalizado}': {e}", exc_info=True)
            return []
    ...
    def obtenerReservasPorUsuario(self, id_propietario: int):
        try:
            ...
            logger.error(f"Error obteniendo reservas para usuario {id_propietario}: {e}", exc_info=True)
            return []
    ...
    def obtenerReservasPorObra(self, id_obra: int):
        try:
            ...
            logger.error(f"Error obteniendo reservas para obra {id_obra}: {e}", exc_info=True)
            return []
    ...
    def obtenerReservasPorUsuarioEstado(self, id_propietario: int, estado: str):
        try:
            ...
            logger.error(f"Error obteniendo reservas para usuario {id_propietario} con estado '{estado}': {e}", exc_info=True)
            return []
    ...
    def obtenerReservasPorExpiracion(self, fecha_expiracion: str):
        try:
            ...
            logger.error(f"Error obteniendo reservas por expiración {fecha_expiracion}: {e}", exc_info=True)
            return []
    ...
    def obtenerReservaPorNombreYObra(self, nombre: str, id_obra: int):
        try:
            ...
            logger.error(f"Error obteniendo reserva por nombre '{nombre_normalizado}' y obra {id_obra}: {e}", exc_info=True)
            return None
    ...
    def actualizarEstadoReserva(self, id_reserva: int, nuevo_estado: str):
        ...
        try:
            ...
            logger.error(f"Error actualizando estado de reserva {id_reserva}: {e}", exc_info=True)
            raise
        
    def editarReserva(self, id_reserva:int, nuevo_id_obra:int, nuevo_id_hilo:int, nuevo_id_mensaje:int):
        try:
            cursor = self.ejecutar('''
            ...
            logger.error(f"Error actualizando mensaje de reserva {id_reserva}: {e}", exc_info=True)
            raise
    ...
    def obtenerReservasVencidasAntiguas(self, dias: int):
        try:
            ...
            logger.error(f"Error obteniendo reservas vencidas antiguas: {e}", exc_info=True)
            return []
    ...
    def marcarReservasVencidasPorUsuario(self, id_usuario: int):
        try:
            ...
            logger.error(f"Error marcando reservas como vencidas para usuario {id_usuario}: {e}", exc_info=True)
            raise
    ...
    def eliminarReservaDefinitiva(self, id_reserva: int):
        try:
            ...
            logger.error(f"Error eliminando definitivamente reserva {id_reserva}: {e}", exc_info=True)
            raise
    ...
    def obtenerAutorDeReserva(self, id_reserva: int):
        try:
            ...
            logger.error(f"Error obteniendo autor de reserva {id_reserva}: {e}", exc_info=True)
            return None