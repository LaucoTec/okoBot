...
class ObraSelec(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not aliases:
            await interaction.response.send_message(f"No hay alias registrados para la obra '{obraData['nombre_obra']}'.", ephemeral=True)
            return
        
        mensaje = f"\n".join([f"- {alias['alias']}" for alias in aliases])
        mensaje = discord.Embed(title=f"Alias de {obraData['nombre_obra']}", description=mensaje, color=discord.Color.blue())
        await interaction.response.edit_message(content=f"Mostrando {len(aliases)} alias.",embed=mensaje)
...
class AliasCog(commands.Cog):
    ...
    aliasGroup = app_commands.Group(name="alias", description="Comandos para gestionar alias de obras")
    ...
    @aliasGroup.command(name="crear", description="Crear un alias para una obra existente")
    ...
    async def crear_alias(self, interaction: discord.Interaction, alias: str, obra: str):
        obraData = self.bot.bd.obras.obtenerObraPorNombre(obra)
        if obraData is None:
            await interaction.response.send_message(f"No se encontró la obra '{obra}'.", ephemeral=True)
            return
        ...
        if idAlias is not None:
            await interaction.response.send_message(f"Alias '{alias}' creado para la obra '{obra}'.", ephemeral=True)
        else:
            await interaction.response.send_message("Error al crear el alias.", ephemeral=True)
    ...
    async def eliminar_alias(self, interaction: discord.Interaction, alias: str):
        ...
        if aliasData is None:
            await interaction.response.send_message(f"No se encontró el alias '{alias}'.", ephemeral=True)
            return

        idAlias = aliasData["id_alias"]
        self.bot.bd.aliasObras.eliminarAliasObra(idAlias)
        await interaction.response.send_message(f"Alias '{alias}' eliminado.", ephemeral=True)
        
    @aliasGroup.command(name="listar", description="Listar todos los alias registrados por obra")
    ...
    async def listar_aliases(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Selecciona una obra para ver sus alias:", view=view, ephemeral=True)
