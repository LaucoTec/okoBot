...
from config import ID_RESERVAS, ID_LOGS_RESERVAS, ID_ADVERTENCIAS
...
class TareasProgramadas(commands.Cog):
    ...
    async def integridadIDs(self):
        ...
        for ficha in fichas:
            if not await self._ids_validos(ficha['id_hilo'], ficha['id_mensaje']):
                if self.bot.bd.fichas.eliminarFichaDefinitiva(ficha['id_ficha']):
                    eliminadas_fichas += 1
                    logger.info(f"Eliminada ficha inválida id_ficha={ficha['id_ficha']} id_hilo={ficha['id_hilo']} id_mensaje={ficha['id_mensaje']}")
        ...
        for reserva in reservas:
            if not await self._ids_validos(reserva['id_hilo'], reserva['id_mensaje']):
                if self.bot.bd.reservas.eliminarReservaDefinitiva(reserva['id_reserva']):
                    eliminadas_reservas += 1
                    logger.info(f"Eliminada reserva inválida id_reserva={reserva['id_reserva']} id_hilo={reserva['id_hilo']} id_mensaje={reserva['id_mensaje']}")
    ...
    async def actualizarEstadosReservas(self):
        ...
        for reserva in reservas:
            ...
            fecha_expiracion = datetime.strptime(reserva['fecha_expiracion'], "%Y-%m-%d").date()
            if fecha_expiracion < hoy:
                # Ya pasó la fecha de expiración
                if self.bot.bd.reservas.actualizarEstadoReserva(reserva['id_reserva'], 'vencida'):
                    embed.color = discord.Color.red()
                    embed.description = f"Reserva expirada desde {reserva['fecha_expiracion']}"
                    actualizadas += 1
                    logger.info(f"Reserva vencida: id_reserva={reserva['id_reserva']} fecha_expiracion={fecha_expiracion}")
            elif fecha_expiracion == hoy and reserva['estado'] == 'activa':
                # Hoy es el último día
                if self.bot.bd.reservas.actualizarEstadoReserva(reserva['id_reserva'], 'por_expirar'):
                    embed.color = discord.Color.orange()
                    actualizadas += 1
                    logger.info(f"Reserva por expirar hoy: id_reserva={reserva['id_reserva']} fecha_expiracion={fecha_expiracion}")
            
            await mensaje.edit(embed=embed)

        logger.info(f"Actualización de estados de reservas completada: {actualizadas} reservas actualizadas.")

    async def eliminarReservasVencidasAntiguas(self, dias: int = 7):
        logger.info(f"Buscando reservas vencidas hace más de {dias} días para eliminar...")
        ...
        if canalLogs is None:
            logger.error(f"No se pudo obtener el canal de logs de reservas: {ID_LOGS_RESERVAS}")
            return
        ...
        for reserva in reservas_vencidas:
            try:
                ...
                if self.bot.bd.reservas.eliminarReservaDefinitiva(reserva['id_reserva']):
                    eliminadas += 1
                    logger.info(f"Reserva vencida eliminada: id_reserva={reserva['id_reserva']}")
            except discord.NotFound:
                logger.warning(f"Mensaje ya no existe para reserva {reserva['id_reserva']}")
            except Exception as e:
                logger.error(f"Error procesando reserva vencida {reserva['id_reserva']}: {e}", exc_info=True)
        ...
        logger.info(f"Eliminación de reservas vencidas completada: {eliminadas} reservas eliminadas.")

    async def eliminarHilosEliminadosAntiguos(self, dias: int = 7):
        logger.info(f"Buscando fichas eliminadas hace más de {dias} días para borrar hilos...")
        ...
        for ficha in fichas_eliminadas:
            ...
            try:
                ...
                await thread.delete(reason=f"Eliminar hilo de ficha eliminada por más de {dias} días")
                eliminados += 1
                logger.info(f"Hilo eliminado: id_hilo={id_hilo} para ficha id_ficha={ficha['id_ficha']}")

            except discord.NotFound:
                no_encontrados += 1
                logger.warning(f"Hilo no encontrado al intentar borrar id_hilo={id_hilo} para ficha id_ficha={ficha['id_ficha']}")
            except discord.Forbidden as e:
                logger.error(f"Permisos insuficientes para borrar hilo id_hilo={id_hilo} de ficha id_ficha={ficha['id_ficha']}: {e}", exc_info=True)
            except Exception as e:
                logger.error(f"Error borrando hilo id_hilo={id_hilo} de ficha id_ficha={ficha['id_ficha']}: {e}", exc_info=True)
    ...
    async def _ids_validos(self, id_hilo: int, id_mensaje: int) -> bool:
        try:
            ...
            logger.error(f"Permiso denegado al verificar ids {id_hilo}/{id_mensaje}: {e}", exc_info=True)
            return False
        except Exception as e:
            logger.error(f"Error verificando IDs {id_hilo}/{id_mensaje}: {e}", exc_info=True)
            return False
            ...
            try:
                ...
                if miembro is None:
                    ...
                    if self.bot.bd.usuarios.eliminarUsuario(id_usuario):
                        eliminados += 1
                        logger.info(f"Usuario eliminado por ausencia: id_usuario={id_usuario}")
            except Exception as e:
                logger.error(f"Error verificando usuario {id_usuario}: {e}", exc_info=True)
        
        logger.info(f"Limpieza de usuarios ausentes completada: {eliminados} usuarios eliminados.")
    
    async def marcarInactivosPorEliminar(self, dias: int = 5):
        logger.info(f"Marcando fichas y reservas de usuarios inactivos (>{dias} días)...")
        ...
        if canal_advertencias is None:
            logger.error(f"No se pudo obtener el canal de advertencias: {ID_ADVERTENCIAS}")
            return
        ...
        for usuario in usuarios_inactivos:
            ...
            try:
                ...
                fichas_count = self.bot.bd.fichas.marcarFichasEliminadasPorUsuario(id_usuario)
                if fichas_count:
                    fichas_marcadas += fichas_count
                
                # Marcar reservas activas como vencidas
                reservas_count = self.bot.bd.reservas.marcarReservasVencidasPorUsuario(id_usuario)
                if reservas_count:
                    reservas_marcadas += reservas_count
                
                # Si se marcaron registros, enviar advertencia
                if fichas_count or reservas_count:
                    await self._enviarAdvertenciaInactividad(canal_advertencias, id_usuario)
                    logger.info(f"Advertencia enviada a usuario inactivo {id_usuario}: {fichas_count} fichas, {reservas_count} reservas")
                
            except Exception as e:
                logger.error(f"Error procesando usuario inactivo {id_usuario}: {e}", exc_info=True)
        
        logger.info(
            f"Marcado de inactivos completado: {fichas_marcadas} fichas marcadas, {reservas_marcadas} reservas marcadas."
        )
    
    async def _enviarAdvertenciaInactividad(self, canal: discord.TextChannel, id_usuario: int):
        try:
            usuario = self.bot.get_user(id_usuario)
            if usuario is None:
                logger.warning(f"No se pudo obtener usuario {id_usuario} para advertencia")
                return
            ...
            logger.error(f"Permisos insuficientes para crear hilo de advertencia para usuario {id_usuario}: {e}", exc_info=True)
        except Exception as e:
            logger.error(f"Error enviando advertencia a usuario {id_usuario}: {e}", exc_info=True)
