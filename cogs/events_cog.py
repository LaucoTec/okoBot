...
class Eventos(commands.Cog):
    ...
    async def on_message(self, mensaje):
        ...
        if ref.message_id == data.get("mensaje_id") and mensaje.channel.id == data.get(
            "canal_id"
        ):
        ...
        if canalGeneral is not None and canalVerificacion is not None:
            try:
                await canalGeneral.send(
                    f"Estimado {miembro.mention}: No olvide completar la verificación en {canalVerificacion.mention} para evitar ser expulsado.",
                    delete_after=20,
                )
            except Exception as e:
                logger.error(
                    f"Error enviando mensaje de bienvenida para {miembro.id}: {e}",
                    exc_info=True,
                )
