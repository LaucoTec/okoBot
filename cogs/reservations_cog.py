...
from discord.app_commands import CommandOnCooldown, MissingPermissions, MissingRole
...
class reservaSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        reserva = next((r for r in self.reservas if r["id_reserva"] == reservaID), None)

        if reserva is None:
            await interaction.response.send_message(
                content="No se encontró la reserva seleccionada.", ephemeral=True
            )
            return
...
class reservasVista(discord.ui.View):
    def __init__(self, bot, reservas, callback_select=None):
        ...
        if self.callbackSelect is not None:
            self.add_item(
                reservaSelect(reservas=self.reservas, callback=self.callbackSelect)
            )
    ...
    def generarEmbed(self):
        ...
        embed = discord.Embed(
            title=reserva["nombre_personaje"], color=discord.Color.blurple()
        )
        ...
        embed.set_footer(text=f"{self.index + 1}/{len(self.reservas)}")
    ...
    async def anterior(self, interaction: discord.Interaction, button):
        ...
        await interaction.response.edit_message(embed=self.generarEmbed(), view=self)
...
class confirmarEliminar(discord.ui.View):
    ...
    async def procesar(self, interaction: discord.Interaction):
        ...
        await interaction.edit_original_response(
            content="Reserva eliminada", embed=self.embed
        )
        ...
        await canalLog.send(content="Reserva eliminada", embed=embedLog)

        self.bot.bd.reservas.actualizarEstadoReserva(
            id_reserva=self.reserva["id_reserva"], nuevo_estado="vencida"
        )
    ...
    async def cancelar(self, interaction: discord.Interaction, button):
        ...
        await interaction.response.edit_message(
            content="Operación cancelada.", embed=self.embed, view=None
        )
        self.stop()
...
class confirmarEditar(discord.ui.View):
    ...
    async def procesar(self, interaction: discord.Interaction):
        ...
        await interaction.edit_original_response(
            content="Reserva movida correctamente.", view=None
        )
        self.stop()
        ...
        embedLog = discord.Embed(title=self.motivo, color=discord.Color.pink())
        embedLog.set_author(
            name=interaction.user.display_name,
            icon_url=interaction.user.display_avatar.url,
        )
        ...
        await canalLog.send(content="Reserva actualizada", embed=embedLog)
    ...
    async def cancelar(self, interaction: discord.Interaction, button):
        await interaction.response.edit_message(
            content="Operación cancelada.", embed=None, view=None
        )
        self.stop()


class reservaModal(discord.ui.Modal, title="Crear Reserva de Apariencia"):
    nombrePj = discord.ui.TextInput(
        label="Nombre del personaje", placeholder="Ej: Steel", required=True
    )
    obra = discord.ui.TextInput(
        label="Obra original", placeholder="Ej: Max Steel (Reboot)", required=True
    )
    placeholder = discord.ui.TextInput(
        label="¿No está el universo de tu personaje?",
    ...
    async def cancelarReserva(self, userID):
        ...
        if userID in self.bot.reservasPendientes:
            ...
            await canal.send(
                f"Reserva pendiente del usuario <@{userID}> ha sido cancelada por falta de respuesta.",
                delete_after=10,
            )
    ...
    def obtenerSimilitud(self, nombrePj, reservasActivas):
        ...
        resultado = process.extractOne(nombrePj, reservasActivas, scorer=scorer)
    ...
    async def revisarDuplicados(self, idObra, nombrePj, interaction):
        ...
        for reserva in reservas:
            if nombreNorm == reserva["nombre_normalizado"]:
                ...
                if (
                    ...
                    and reserva["id_propietario"] == idUsuario
                ):
        ...
        reservasPersonales = {
            r["nombre_normalizado"]: r["nombre_personaje"]
            for r in reservas
            if r["id_propietario"] == idUsuario and r["estado"] == "vencida"
        }
        if reservasPersonales:
            _, puntuacionPersonal = self.obtenerSimilitud(
                nombrePj=nombreNorm, reservasActivas=reservasPersonales
            )
        ...
        if reservasActivas:
            mejorCoincidencia, puntuacion = self.obtenerSimilitud(
                nombrePj=nombreNorm, reservasActivas=reservasActivas
            )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if "solicitar" in obraIngresada.lower():
            ...
            mensaje += (
                "Mientras tanto, tu reserva estará guardada en un canal temporal.\n\n"
            )
        ...
        embed = discord.Embed(
            title=f"Reserva recibida", color=discord.Color.brand_green()
        )
        contenido = f"Nombre: {nombre}\nObra: {obra['nombre_obra']}\n"

        estado, duplicado = await self.revisarDuplicados(
            idObra=obra["id_obra"], nombrePj=nombre, interaction=interaction
        )
        if estado == "BLOQUEO_OCUPADO":
            ...
            await interaction.response.send_message(
                content=mensaje, embed=embed, delete_after=20, ephemeral=True
            )
            return
            ...
            contenido += (
                "- No hay ninguna reserva colisionando, puede seguir sin problemas.\n"
            )
        ...
        await interaction.response.send_message(
            content=mensaje, embed=embed, delete_after=120
        )
...
class ReservaCog(commands.Cog):
    ...
    reservaGroup = app_commands.Group(
        name="reserva", description="Comandos para gestionar reservas de apariencia"
    )
    ...
    async def procesarImagen(self, mensaje: discord.Message):
        ...
        if not imagen.content_type or not imagen.content_type.startswith("image/"):
            await mensaje.channel.send(
                "Por favor, responde con una imagen válida para completar la reserva.",
                delete_after=10,
            )
            return
        ...
        mensajeReservas = await canal.send(
            content=f"Reserva de {mensaje.author} (ID: {mensaje.author.id})",
            file=archivo,
        )
    ...
    async def registrarReserva(self, data: dict):
        try:
            ...
            msj = await self.embedReserva(
                data=data, fechaInicio=fechaInicio, fechaFin=fechaFin
            )
            msjLog = await self.embedLog(
                data=data, fechaInicio=fechaInicio, fechaFin=fechaFin
            )
            ...
            self.bot.bd.reservas.editarMensajeIdReserva(
                id_reserva=idReserva, nuevo_id_mensaje=reserva.id
            )
    ...
    async def embedReserva(self, data: dict, fechaInicio, fechaFin):
        ...
        embed = discord.Embed(
            title=data["nombre_personaje"], color=discord.Color.green()
        )
        embed.set_author(
            name=data["autor"].display_name, icon_url=data["autor"].display_avatar.url
        )

        if "solicitar" in data["obraIngresada"].lower():
            obraIngresada = (
                data["obraIngresada"].lower().replace("solicitar", "").strip()
            )
            embed.description = (
            ...
            embed.description = f"**Inicio:** {inicioStr}\n" f"**Expira:** {finStr}"
    ...
    async def autocompletarReserva(
        ...
        for r in reservas:
            if (
                current.lower() in r["nombre_personaje"].lower()
                and r["estado"] != "vencida"
            ):
                resultados.append(
                    app_commands.Choice(
                        name=r["nombre_personaje"], value=r["id_reserva"]
                    )
                )
    ...
    async def autocompletarReservaRenovada(
        ...
        reservas = bd.reservas.obtenerReservasPorUsuarioEstado(
            id_propietario=userID, estado="por_expirar"
        )
        resultados = []
        for r in reservas:
    ...
    async def callbackEliminarAdmin(self, interaction, reserva):
        embed = discord.Embed(title="Confirmación", color=discord.Color.orange())
        ...
        embed.set_thumbnail(url=reserva["enlace_imagen"])

        botones = confirmarEliminar(bot=self.bot, reserva=reserva, embed=embed)

        await interaction.response.edit_message(
            content="Eliminación de reserva", embed=embed, view=botones
        )
    ...
    @reservaGroup.command(
        name="crear", description="Crear una nueva reserva de apariencia"
    )
    ...
    @app_commands.describe(
        id_reserva="El ID de la reserva a editar",
        obra_nueva="Nombre o alias de la obra destino",
        motivo="(Opcional) Razón para el cambio.",
    )
    @app_commands.default_permissions(administrator=True)
    async def editarReserva(
        self,
        interaction: discord.Interaction,
        id_reserva: int,
        obra_nueva: str,
        motivo: str = "Obra previamente no creada.",
    ):
        ...
        embed = discord.Embed(
            title="Confirmar cambio de obra.", color=discord.Color.yellow()
        )
        embed.add_field(
            name="Reserva", value=f"{reserva['nombre_personaje']}", inline=False
        )
        embed.add_field(
            name="Obra actual", value=f"{obraViejaData['nombre_obra']}", inline=True
        )
        embed.add_field(
            name="Obra nueva", value=f"{obraNuevaData['nombre_obra']}", inline=True
        )

        botones = confirmarEditar(
            bot=self.bot,
            reserva=reserva,
            obraNueva=obraNuevaData,
            obraVieja=obraViejaData,
            motivo=motivo,
        )

        await interaction.response.send_message(
            content="Editando una reserva...", embed=embed, view=botones, ephemeral=True
        )
    ...
    async def eliminarReserva(self, interaction: discord.Interaction, id_reserva: int):
        ...
        if reserva is None:
            await interaction.response.send_message(
                content="No se encontró la reserva.", ephemeral=True
            )
            return
        if reserva["id_propietario"] != interaction.user.id:
            await interaction.response.send_message(
                content="No puedes eliminar reservas de otros usuarios.", ephemeral=True
            )
            return
    ...
    async def eliminarReservaAdmin(
        ...
        vista = reservasVista(
            bot=self.bot, reservas=reservas, callback_select=self.callbackEliminarAdmin
        )
    ...
    @reservaGroup.command(
        name="listar",
        description="Muestra las reservas de un usuario u obra. Prioriza obra.",
    )
    @app_commands.describe(
        usuario="El usuario a obtener sus reservas",
        obra="La obra a obtener sus reservas",
    )
    @app_commands.autocomplete(obra=autocompletarObra)
    @app_commands.checks.cooldown(1, 20.0, key=lambda i: (i.user.id))
    async def listarReserva(
        self,
        interaction: discord.Interaction,
        ...
        vista = reservasVista(bot=self.bot, reservas=reservas)
    ...
    @reservaGroup.command(
        name="renovar",
        description="Renueva una reserva propia el día de su vencimiento.",
    )
    ...
    async def renovarReserva(self, interaction: discord.Interaction, reserva: int):
        ...
        embed = discord.Embed(
            title="Renovación de reserva", color=discord.Color.greyple()
        )
        embed.description = (
            "> *Datos*\n\n"
        ...
        if self.bot.bd.reservas.renovarReserva(
            id_reserva=reserva, fecha_fin=fechaFin.date().isoformat()
        ):
            inicioStr = fechaInicio.date().strftime("%d/%m/%Y")
            finStr = fechaFin.date().strftime("%d/%m/%Y")
            ...
            if self.bot.bd.reservas.editarMensajeIdReserva(
                id_reserva=reserva, nuevo_id_mensaje=mensajeNuevo.id
            ):
                await interaction.response.send_message(
                    content="Reserva renovada.", embed=embed, delete_after=300
                )

                await mensajeOg.delete()

                description = (
                    f"**Personaje**: {renovar['nombre_personaje']}\n"
                    + embedNuevo.description
                )
                embedLog = discord.Embed(
                ...
                if canalLog is None:
                    canalLog = await self.bot.fetch_channel(ID_LOGS_RESERVAS)
                await canalLog.send(content="Reserva renovada", embed=embedLog)
